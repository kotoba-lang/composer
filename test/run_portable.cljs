...
(require '[cljs.test :as t]
         '[kotoba.composer-test]
         '[kotoba.composer.generation-test]
         '[kotoba.composer.pipeline-test]
         '[kotoba.composer.stem-test]
         '[kotoba.composer.style-test]
         '[kotoba.composer.track-test])

(defmethod t/report [:cljs.test/default :end-run-tests] [m]
  (when-not (t/successful? m) (set! (.-exitCode js/process) 1)))

(t/run-tests 'kotoba.composer-test
              'kotoba.composer.generation-test
              'kotoba.composer.pipeline-test
              'kotoba.composer.stem-test
              'kotoba.composer.style-test
